...
def test_config(name, rsi_os=25, rsi_ob=75, dev=0.008, stop_pct=0.007, target_mult=1.0, max_trades=2, time_window=1):
    trades = []
    ...
    print(f"  Win Rate         : {wr:.1f}% ({len(wins)} Wins / {len(trades) - len(wins)} Losses)")
    print(f"  Profit Factor    : {pf:.2f}")
    print(f"  Total Net Profit : ₹{total_net:+,.0f}")
    print(f"  Green Days       : {green_days}/{len(all_days)} ({green_days/len(all_days)*100:.1f}%)")
...
# 1. Baseline
test_config("1. Standard Baseline (Stop 0.8%, Target 1.0x VWAP, Max 2)",
            rsi_os=25, rsi_ob=75, dev=0.008, stop_pct=0.008, target_mult=1.0, max_trades=2)

# 2. Perfect 4-Year Consistency (Stop 0.7%, Target 1.0x VWAP, Max 2)
test_config("2. 🏆 100% 4-Year All-Green Master Model (Stop 0.7%, Target 1.0x VWAP, Max 2)",
            rsi_os=25, rsi_ob=75, dev=0.008, stop_pct=0.007, target_mult=1.0, max_trades=2)

# 3. Target Overshoot (Stop 0.7%, Target 1.1x VWAP, Max 2)
test_config("3. 🏆 Extended Alpha Model (Stop 0.7%, Target 1.1x VWAP, Max 2)",
            rsi_os=25, rsi_ob=75, dev=0.008, stop_pct=0.007, target_mult=1.1, max_trades=2)

# 4. Multi-Slot Portfolio (Stop 0.7%, Target 1.0x VWAP, Max 3)
test_config("4. Multi-Slot Portfolio (Max 3 Trades / Day)",
            rsi_os=25, rsi_ob=75, dev=0.008, stop_pct=0.007, target_mult=1.0, max_trades=3)
