import yfinance as yf
import pandas as pd
import numpy as np
from collections import defaultdict

TICKERS = [
    "RELIANCE.NS", "TCS.NS", "HDFCBANK.NS", "ICICIBANK.NS", "INFY.NS",
    "SBIN.NS", "AXISBANK.NS", "KOTAKBANK.NS", "LT.NS", "BHARTIARTL.NS",
    "BAJFINANCE.NS", "WIPRO.NS", "HCLTECH.NS", "TITAN.NS", "SUNPHARMA.NS",
    "MARUTI.NS", "ADANIENT.NS", "NTPC.NS", "POWERGRID.NS"
]

data_1h = {}
for t in TICKERS:
    try:
        df = yf.download(t, period="730d", interval="1h", auto_adjust=True, progress=False, multi_level_index=False)
        if not df.empty:
            df.columns = [c.lower() for c in df.columns]
            if df.index.tz is not None: df.index = df.index.tz_localize(None)
            data_1h[t] = df.dropna()
    except Exception: pass

# Also daily data for multi-day regime filtering (200 SMA, 20-day high breakout)
data_d = {}
for t in TICKERS:
    try:
        df_d = yf.download(t, period="3y", interval="1d", auto_adjust=True, progress=False, multi_level_index=False)
        if not df_d.empty:
            df_d.columns = [c.lower() for c in df_d.columns]
            if df_d.index.tz is not None: df_d.index = df_d.index.tz_localize(None)
            data_d[t] = df_d.dropna()
    except Exception: pass

def vwap_hourly(df):
    df = df.copy()
    df["date"] = df.index.date
    df["tp"] = (df["high"] + df["low"] + df["close"]) / 3
    df["tpv"] = df["tp"] * df["volume"]
    return df.groupby("date")["tpv"].cumsum() / df.groupby("date")["volume"].cumsum()

def adx_calc(df, period=14):
    high, low, close = df['high'], df['low'], df['close']
    prev_close = close.shift(1)
    plus_dm = high.diff().clip(lower=0)
    minus_dm = (-low.diff()).clip(lower=0)
    plus_dm[high.diff() <= -low.diff()] = 0
    minus_dm[-low.diff() <= high.diff()] = 0
    tr = pd.concat([high - low, (high - prev_close).abs(), (low - prev_close).abs()], axis=1).max(axis=1)
    a = tr.ewm(com=period - 1, adjust=False).mean()
    plus_di = 100 * plus_dm.ewm(com=period - 1, adjust=False).mean() / a.replace(0, np.nan)
    minus_di = 100 * minus_dm.ewm(com=period - 1, adjust=False).mean() / a.replace(0, np.nan)
    dx = 100 * (plus_di - minus_di).abs() / (plus_di + minus_di).replace(0, np.nan)
    return dx.ewm(com=period - 1, adjust=False).mean().fillna(0)

# Precalculate indicators
precalc = {}
for t, df in data_1h.items():
    df_d = data_d.get(t, pd.DataFrame())
    precalc[t] = {
        "vwap": vwap_hourly(df),
        "adx": adx_calc(df),
        "d_sma200": df_d['close'].rolling(200).mean() if not df_d.empty else pd.Series(),
        "d_sma20": df_d['close'].rolling(20).mean() if not df_d.empty else pd.Series(),
        "d_close": df_d['close'] if not df_d.empty else pd.Series(),
    }

all_days = sorted(set(list(data_1h.values())[0].index.date))
COST_PER_TRADE = 50
RISK_PER_TRADE = 2000

def test_system(name, min_adx=22, rr=2.0, require_daily_trend=True, max_t=1, only_long=False, gap_filter=True):
    trades = []
    daily_pnl = defaultdict(float)
    yearly_pnl = defaultdict(float)

    for day in all_days:
        day_signals = []
        for t, df in data_1h.items():
            today = df[df.index.date == day]
            if len(today) < 3: continue
            
            # Daily Trend Filter
            d_s200 = precalc[t]['d_sma200']
            d_s20 = precalc[t]['d_sma20']
            d_c = precalc[t]['d_close']
            
            # Find yesterday's daily values
            past_d = d_c.loc[:pd.Timestamp(day)]
            if len(past_d) < 200: continue
            prev_day_close = past_d.iloc[-2]
            sma200_val = d_s200.loc[:pd.Timestamp(day)].iloc[-2]
            sma20_val = d_s20.loc[:pd.Timestamp(day)].iloc[-2]
            
            is_bullish_regime = prev_day_close > sma200_val and prev_day_close > sma20_val
            is_bearish_regime = prev_day_close < sma200_val and prev_day_close < sma20_val
            
            r_hi = today.iloc[0]['high']
            r_lo = today.iloc[0]['low']
            rng = r_hi - r_lo
            c_open = today.iloc[0]['close']
            
            if rng / c_open > 0.025 or rng / c_open < 0.003: continue
            
            # Gap filter: don't chase >1.5% gaps
            gap = abs(today.iloc[0]['open'] - prev_day_close) / prev_day_close
            if gap_filter and gap > 0.015: continue
            
            adx_val = precalc[t]['adx'].loc[today.index[0]] if today.index[0] in precalc[t]['adx'].index else 0
            if adx_val < min_adx: continue
            
            vw = precalc[t]['vwap']
            
            for i in range(1, min(4, len(today))):
                bar = today.iloc[i]
                prev = today.iloc[i-1]
                vw_val = vw.loc[today.index[i]]
                
                # LONG: Must be in bullish daily regime
                if bar['close'] > r_hi and prev['close'] <= r_hi and bar['close'] > vw_val:
                    if require_daily_trend and not is_bullish_regime: break
                    stop = r_lo
                    risk = bar['close'] - stop
                    if risk > 0:
                        target = bar['close'] + rr * risk
                        day_signals.append({
                            'ticker': t, 'dir': 'LONG', 'entry': bar['close'], 'stop': stop,
                            'target': target, 'bar_idx': i, 'score': adx_val, 'risk': risk
                        })
                    break
                    
                # SHORT: Must be in bearish daily regime
                if not only_long and bar['close'] < r_lo and prev['close'] >= r_lo and bar['close'] < vw_val:
                    if require_daily_trend and not is_bearish_regime: break
                    stop = r_hi
                    risk = stop - bar['close']
                    if risk > 0:
                        target = bar['close'] - rr * risk
                        day_signals.append({
                            'ticker': t, 'dir': 'SHORT', 'entry': bar['close'], 'stop': stop,
                            'target': target, 'bar_idx': i, 'score': adx_val, 'risk': risk
                        })
                    break

        selected = sorted(day_signals, key=lambda x: x['score'], reverse=True)[:max_t]
        
        for sig in selected:
            t = sig['ticker']
            df = data_1h[t]
            today = df[df.index.date == day]
            entry, stop, target, direction, bar_idx, risk = sig['entry'], sig['stop'], sig['target'], sig['dir'], sig['bar_idx'], sig['risk']
            qty = max(1, int(RISK_PER_TRADE / risk))
            curr_stop = stop
            exit_price = None
            result = 'squareoff'
            
            for j in range(bar_idx + 1, len(today)):
                bar = today.iloc[j]
                if j == len(today) - 1:
                    exit_price = bar['close']
                    result = 'squareoff'
                    break
                    
                # Breakeven trailing stop at +1R
                if direction == 'LONG' and bar['high'] >= entry + risk:
                    curr_stop = max(curr_stop, entry + 0.1 * risk)
                elif direction == 'SHORT' and bar['low'] <= entry - risk:
                    curr_stop = min(curr_stop, entry - 0.1 * risk)
                    
                if direction == 'LONG':
                    if bar['low'] <= curr_stop:
                        exit_price = curr_stop
                        result = 'stop'
                        break
                    if bar['high'] >= target:
                        exit_price = target
                        result = 'target'
                        break
                else:
                    if bar['high'] >= curr_stop:
                        exit_price = curr_stop
                        result = 'stop'
                        break
                    if bar['low'] <= target:
                        exit_price = target
                        result = 'target'
                        break
                        
            if exit_price is None: continue
            pnl = (exit_price - entry) * qty if direction == 'LONG' else (entry - exit_price) * qty
            net_pnl = pnl - COST_PER_TRADE
            trades.append({"net_pnl": net_pnl, "result": result, "ticker": t, "date": str(day), "dir": direction})
            daily_pnl[day] += net_pnl
            yearly_pnl[day.year] += net_pnl

    if not trades: return
    wins = [t for t in trades if t['net_pnl'] > 0]
    total_net = sum(t['net_pnl'] for t in trades)
    win_rate = len(wins) / len(trades) * 100
    profit_days = sum(1 for v in daily_pnl.values() if v > 0)
    loss_days = sum(1 for v in daily_pnl.values() if v < 0)
    gross_win = sum(t['net_pnl'] for t in wins)
    gross_loss = abs(sum(t['net_pnl'] for t in trades if t['net_pnl'] <= 0))
    pf = gross_win / gross_loss if gross_loss > 0 else 99

    print(f"\n[{name}]")
    print(f"  Trades: {len(trades)} | Win Rate: {win_rate:.1f}% | Net P&L: ₹{total_net:+,.0f} | PF: {pf:.2f}")
    print(f"  Profit Days: {profit_days}/{len(all_days)} ({profit_days/len(all_days)*100:.1f}%) | Loss Days: {loss_days}")
    print(f"  Yearly: " + ", ".join([f"{y}: ₹{p:+,.0f}" for y, p in sorted(yearly_pnl.items())]))

configs = [
    ("1. ADX>20 + Daily 200/20 SMA Filter + Both Sides + MaxT 1", {"min_adx": 20, "rr": 2.0, "require_daily_trend": True, "max_t": 1, "only_long": False}),
    ("2. ADX>25 + Daily 200/20 SMA Filter + Long Only + MaxT 1", {"min_adx": 25, "rr": 2.0, "require_daily_trend": True, "max_t": 1, "only_long": True}),
    ("3. ADX>22 + Daily 200/20 SMA Filter + Long Only + MaxT 2", {"min_adx": 22, "rr": 2.0, "require_daily_trend": True, "max_t": 2, "only_long": True}),
    ("4. ADX>25 + Daily 200 SMA Filter + RR 1.5 + Long Only", {"min_adx": 25, "rr": 1.5, "require_daily_trend": True, "max_t": 1, "only_long": True}),
]

for name, cfg in configs:
    test_system(name, **cfg)
