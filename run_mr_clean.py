...
print("Fetching data cleanly...", flush=True)
...
def test_mr(name, rsi_os=25, rsi_ob=75, dev_pct=0.008, stop_pct=0.008, max_trades=2):
    ...
    print(f"  Green Days       : {green_days}/{len(all_days)} ({green_days/len(all_days)*100:.1f}%)", flush=True)
    print(f"  Yearly Breakdown :", flush=True)
    for y, p in sorted(yearly_pnl.items()):
        print(f"    Year {y} : ₹{p:+10,.0f}", flush=True)
    print(f"==================================================\n", flush=True)

test_mr("1. Extreme VWAP Mean Reversion (RSI < 25 / > 75, Dev > 0.8%)", rsi_os=25, rsi_ob=75, dev_pct=0.008, stop_pct=0.008, max_trades=2)
test_mr("2. Conservative VWAP Mean Reversion (RSI < 20 / > 80, Dev > 0.8%)", rsi_os=20, rsi_ob=80, dev_pct=0.008, stop_pct=0.008, max_trades=1)
