...
print(f"Fetching data for {len(TICKERS)} liquid tickers in bulk...", flush=True)
...
print(f"Dataset ready: {len(data_1h)} tickers across {len(all_days)} trading days (730d range).", flush=True)

def run_test(name, max_trades=2, use_cpr=True):
    trades = []
    ...
    print(f"  Profitable Days  : {profit_days}/{len(all_days)} ({profit_days/len(all_days)*100:.1f}%)", flush=True)
    print(f"  Losing Days      : {loss_days}", flush=True)
    print(f"  Yearly Breakdown :", flush=True)
    for y, p in sorted(yearly_pnl.items()):
        print(f"    Year {y} : ₹{p:+10,.0f}", flush=True)
    print(f"==================================================\n", flush=True)

run_test("Standard ORB (No CPR)", max_trades=2, use_cpr=False)
run_test("Upgraded ORB + CPR Priority Confluence", max_trades=2, use_cpr=True)
